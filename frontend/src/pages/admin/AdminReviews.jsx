              ...
              <tbody>
                {reviews.map((r) => (
                  <tr key={r._id} className="border-t border-gray-200 dark:border-gray-700">
                    ...
                    <td className="p-3 inline-flex items-center gap-1">
                      <svg className="w-4 h-4 text-amber-500 fill-amber-500" viewBox="0 0 24 24" fill="currentColor" aria-hidden><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
                      {r.rating}
                    </td>
                    <td className="p-3 max-w-xs truncate">{r.comment || '-'}</td>
                    <td className="p-3 text-right">
                      <button type="button" onClick={() => handleDelete(r._id)} className="text-theme">Sil</button>
                    </td>
                  </tr>
                ))}
              </tbody>
